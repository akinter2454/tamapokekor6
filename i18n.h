...
// Supported UI languages. Korean is appended (rather than inserted) so language
// values already saved in NVS by older firmware keep the same meaning.
enum Lang : uint8_t { LANG_ES = 0, LANG_EN, LANG_FR, LANG_DE, LANG_IT, LANG_PT, LANG_KO, LANG_COUNT };
#define LANG_DEFAULT LANG_KO  // Korean edition: Korean on a fresh install
...
enum StrId : uint8_t {
  ...
  S_TR_DEF,         // row: defence gauge timing
  S_TR_DEF_HINT,    // defence gauge timing hint
...
// Display-only localization helpers. Canonical battle/save data remains unchanged.
